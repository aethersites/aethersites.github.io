...
(function () {
  // ========== CONFIG ==========
  var TARGET = '/under-construction';       // page to redirect to
  var PASSCODE = '108492';                 // PASSCODE (user-provided)
  var BYPASS_KEY = 'site_bypass_v1';       // localStorage key
  var BYPASS_TTL_DAYS = 7;                 // how long bypass lasts (days)
  var SKIP_EXT = /\.(css|js|png|jpg|jpeg|gif|svg|ico|json|xml|txt|woff2?|ttf|map)$/i;
  var SKIP_PATH_PREFIXES = ['/api/', '/wp-json/', '/admin/', '/wp-admin/', '/robots.txt', '/sitemap.xml'];
  // ============================
  ...
  try {
    ...
    // Helper: read bypass and check expiry
    function hasValidBypass() {
      try {
        var raw = localStorage.getItem(BYPASS_KEY);
        if (!raw) return false;
        var obj = JSON.parse(raw);
        if (!obj || !obj.val) return false;
        if (obj.exp && Date.now() > obj.exp) { localStorage.removeItem(BYPASS_KEY); return false; }
        return true;
      } catch (e) { return false; }
    }

    // Helper: set bypass with TTL (days)
    function setBypass(days) {
      var exp = Date.now() + (days * 24 * 60 * 60 * 1000);
      localStorage.setItem(BYPASS_KEY, JSON.stringify({ val: 1, exp: exp }));
    }

    // Helper: clear bypass
    function clearBypass() { localStorage.removeItem(BYPASS_KEY); }

    // If this is an asset or an API path, do nothing
    if (SKIP_EXT.test(p)) return;
    for (var i = 0; i < SKIP_PATH_PREFIXES.length; i++) {
      if (p.indexOf(SKIP_PATH_PREFIXES[i]) === 0) return;
    }

    // If already on target or a subpath of it:
    if (p === TARGET || p.indexOf(TARGET + '/') === 0) {
      // If bypass is already set but there is a stored original location, go back
      try {
        if (hasValidBypass()) {
          var redirectBack = sessionStorage.getItem('redirect_after_bypass');
          if (redirectBack) {
            sessionStorage.removeItem('redirect_after_bypass');
            // go back to original page
            location.replace(redirectBack);
            return;
          }
        }
      } catch (e) { /* ignore */ }

      // Show passcode UI on the /under-construction page (only if bypass not present)
      if (!hasValidBypass()) showPasscodeUI();
      return; // do not redirect further when already at the target
    }

    // Bypass via query param ?preview=1 OR local bypass
    try {
    ...
    // Not bypassed and not on target -> store current URL so we can return after passcode,
    // then replace location with the target (use replace so Back doesn't go to blocked page)
    try {
      sessionStorage.setItem('redirect_after_bypass', location.href);
    } catch (e) { /* ignore */ }
    location.replace(TARGET + q + h);

    // ------------------
    // UI: passcode modal shown on /under-construction
    // ------------------
    function showPasscodeUI() {
      // If UI already present, don't duplicate
      if (document.getElementById('uc-passcode-modal')) return;

      var css = '\
#uc-passcode-modal { position: fixed; inset: 0; display:flex; align-items:center; justify-content:center; z-index:999999; }\
#uc-passcode-backdrop { position: absolute; inset:0; background: rgba(0,0,0,0.45); }\
#uc-passcode-card { position: relative; min-width:320px; max-width:520px; padding:18px; border-radius:8px; box-shadow:0 8px 30px rgba(0,0,0,0.25); background: #fff; font-family: system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial; }\
#uc-passcode-card h3{ margin:0 0 8px 0; font-size:16px }\
#uc-passcode-card p{ margin:0 0 12px 0; font-size:13px; color:#333 }\
#uc-passcode-card input{ width:100%; padding:8px 10px; font-size:14px; margin-bottom:10px; box-sizing:border-box }\
#uc-passcode-card .uc-actions{ display:flex; gap:8px; justify-content:flex-end }\
#uc-passcode-card button{ padding:8px 12px; border-radius:6px; border:1px solid #ccc; background:#f7f7f7; cursor:pointer }\
#uc-passcode-card button.primary{ background:#111; color:#fff; border-color: #111 }\
#uc-passcode-card .uc-small{ font-size:12px; color:#666; margin-top:8px }\
#uc-passcode-card a.uc-clear{ color:#c33; cursor:pointer; text-decoration:underline; }\
';
      var style = document.createElement('style'); style.appendChild(document.createTextNode(css));
      document.head.appendChild(style);

      var modal = document.createElement('div'); modal.id = 'uc-passcode-modal';
      modal.innerHTML = '\
<div id="uc-passcode-backdrop"></div>\
<div id="uc-passcode-card" role="dialog" aria-modal="true">\
  <h3>Site is under construction</h3>\
  <p>If you have the passcode you can access the site. Enter passcode to continue.</p>\
  <input id="uc-passcode-input" type="password" placeholder="Enter passcode" autocomplete="off" />\
  <div class="uc-actions">\
    <button id="uc-passcode-cancel">Close</button>\
    <button id="uc-passcode-submit" class="primary">Unlock</button>\
  </div>\
  <div class="uc-small">Or add <code>?preview=1</code> to the URL to bypass temporarily. <span style="float:right"><a class="uc-clear">Clear saved bypass</a></span></div>\
</div>';
      document.body.appendChild(modal);

      var input = document.getElementById('uc-passcode-input');
      var submit = document.getElementById('uc-passcode-submit');
      var cancel = document.getElementById('uc-passcode-cancel');
      var clearLink = modal.querySelector('a.uc-clear');
      ...
      function unlock() {
        var v = input.value || '';
        if (v === PASSCODE) {
          setBypass(BYPASS_TTL_DAYS);
          try {
            var red = sessionStorage.getItem('redirect_after_bypass');
            if (red) {
              sessionStorage.removeItem('redirect_after_bypass');
          ...
          // hide modal if no redirect exists
          modal.remove();
        } else {
          input.value = '';
          input.focus();
          // small shake effect:
          var card = document.getElementById('uc-passcode-card');
          card.style.transition = 'transform 0.08s';
          card.style.transform = 'translateX(-8px)';
          setTimeout(function(){ card.style.transform = 'translateX(8px)'; }, 80);
          setTimeout(function(){ card.style.transform = 'translateX(0)'; }, 160);
        }
      }
      ...
      clearLink.addEventListener('click', function(e){ e.preventDefault(); clearBypass(); alert('Saved bypass cleared.'); });
      // clicking backdrop closes
      document.getElementById('uc-passcode-backdrop').addEventListener('click', function(){ modal.remove(); });
    } // showPasscodeUI

  } catch (err) {
    // Fail silently to avoid breaking the site
    console.error('Under-construction redirect script error:', err);
  }
})();
</script>
