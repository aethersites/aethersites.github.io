...
document.addEventListener("DOMContentLoaded", () => {
  // 💡 Grab only dessert recipes from the global list (recipes.js must be loaded first)
  // This makes the rest of the code only use dessert recipes for all filters, searches, etc.
  const dessertRecipes = (recipes || []).filter(r =>
    (r.mealtype ?? r.mealType ?? '').toString().trim().toLowerCase() === 'dessert'
  );
  // Overwrite the global recipes variable so the rest of the code works as before
  window.recipes = dessertRecipes;
